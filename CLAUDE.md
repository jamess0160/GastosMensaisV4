...
- **The parent is checked, never trusted** (`sections/CategoryTree.section.ts`). `IdParentCategory` arrives from the client and is sequential, so `assertParent` requires it to be in the *visible* set (own workspace or global), and on `PUT` it also walks up from the proposed parent to refuse a cycle — a cycle leaves the branch with no root, and `buildTree` would drop it from the response while it stayed live and assignable by id. Depth is not capped. On update, `IdParentCategory` is read with `in body`: `undefined` keeps the parent, `null` promotes to root, and only the first is "no change".
- **Archiving takes the subtree with it.** `DELETE` is `Active = false` over the branch (`getBranch`), scoped by `IdWorkspace` in the `UPDATE` itself, for the same reason archiving an account archives its payment methods: a child left active under an archived parent vanishes from the tree at build time yet is still accepted on a new expense. The `GET` guards the other end too — a node whose parent isn't in the visible list is promoted to root instead of silently dropped, because the tree is a view of the flat list and nothing may disappear in the rendering.

`docs/ROADMAP- MVP.md` maps that remaining work in stages, ordered by the foreign keys, and records the cross-cutting decisions that have to be settled before the movement tables get routes — account balance is already settled (computed, never cached), still open are who writes the derived `Expenses.Status` and the shared period-filter format. Read it before starting a new feature — building out of order means writing against a table whose parent has no API yet.
...
- **`TestDatabase`** wraps the test connection: `truncate([...])` (per suite, CASCADE + restart identity), `connection()` for asserting the persisted row, `reset()`/`migrate()`. Every destructive helper goes through `TestEnv.assertTestDatabase()`, which refuses to run unless the database name contains `test` (override with `TEST_DB_UNSAFE=true`). **`truncate(["Users"])` cascades through `Workspaces` into `Categories`, and that empties the whole table — the 13 global rows from the seed migration included, since they live there with a null `IdWorkspace`.** `globalSetup` only re-runs the migrations once per execution, so the first suite to truncate takes the seed away from every suite after it: a suite that needs a global category seeds its own (`Categories.tests.ts` does) instead of relying on the migration.
