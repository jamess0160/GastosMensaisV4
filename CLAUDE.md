...
Each feature lives under `routes/<Feature>/` (see `routes/Users/`, `routes/Cache/`, `routes/Utils/`) with a consistent split:
...
- `<Feature>.model.ts` — a `class_X_model extends BaseModel` (`Utils/Base.ts`) exposing the injected `KnexConnection` as `this.KnexConnection`; holds the Knex queries for one primary table.
...
- `Utils/Connections/Knex/KnexConnection.ts` builds the shared `KnexConnection` from encrypted env vars, and exports `KnexTransaction(fn)` — wraps `knex.transaction`, passing a `TransactionEvents` (`section/transactionEvents.ts`) so code inside a transaction can register `attachOnEnd(fn)` callbacks that fire after the transaction resolves.
