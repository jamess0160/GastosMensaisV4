...
da leva 3" são coisas diferentes. Por isso **um documento nunca cita a etapa de outro**: a
referência cruzada é sempre de **leva**, e o que precisa ser dito sobre a etapa se diz pelo nome
do que ela entrega. Aqui dentro, "etapa 3" sem qualificação é sempre desta leva.
...
| Etapa | O que é | Pendência |
| --- | --- | --- |
| **1** | [Logout](#etapa-1--logout) | [1](../Pendencias%20Backend.md#1-logout) |
| **2** | [Convite para o workspace](#etapa-2--convite-para-o-workspace) | [2](../Pendencias%20Backend.md#2-idworkspace-no-cadastro-aceito-sem-convite) |
| **3** | [`Occurrences` sai do corpo do gasto](#etapa-3--occurrences-sai-do-corpo-do-gasto) | [10](../Pendencias%20Backend.md#10-occurrences-fora-do-post-expenses) |
| **4** | [`Brand` e `LastDigits` saem do cartão](#etapa-4--brand-e-lastdigits-saem-do-cartão) | [16](../Pendencias%20Backend.md#16-bandeira-e-final-do-cartão-saem-do-cadastro) |
| **5** | [Desfazer recebimento de entrada](#etapa-5--desfazer-recebimento-de-entrada) | [14](../Pendencias%20Backend.md#14-desfazer-recebimento-de-entrada) |
| **6** | [Criação de entradas em lote](#etapa-6--criação-de-entradas-em-lote) | [15](../Pendencias%20Backend.md#15-criação-de-entradas-em-lote) |
| **7** | [As pernas de um período](#etapa-7--as-pernas-de-um-período) | [12](../Pendencias%20Backend.md#12-lista-de-gastos-com-os-filhos) |
| **8** | [Conta "apenas cartão", e quem aceita cartão de crédito](#etapa-8--conta-apenas-cartão-e-quem-aceita-cartão-de-crédito) | — *demanda de produto* |
| **9** | [Orçamento por pessoa](#etapa-9--orçamento-por-pessoa) | — *demanda de produto* |
| **10** | [O cartão ganha os dois fatos que faltavam](#etapa-10--o-cartão-ganha-os-dois-fatos-que-faltavam) | — *demanda de produto* |
| **11** | [Estorno: gasto negativo no crédito](#etapa-11--estorno-gasto-negativo-no-crédito) | — *demanda de produto* |

E duas pendências **não viram etapa**:

| Pendência | Por quê |
| --- | --- |
| [11 — Clonar o mês anterior de Renda](../Pendencias%20Backend.md#11-clonar-o-mês-anterior-de-renda) | Já **substituída** pela 15 no próprio documento do front: a escolha do que clonar passou para o usuário, e o que sobrou para o servidor é gravar em lote — a **etapa 6**. Fica registrada para ninguém implementar `POST /Inflows/clone` mais tarde achando que ficou esquecida |
...
- **Esta etapa não depende de envio de e-mail.** O servidor devolve o hash e **quem entrega o
  link é o usuário**. É o que a mantém fora da dependência que trava a recuperação de senha, na leva 3 —
...
- **O rateio que acompanha a perna é o do gasto, não da perna.** Numa compra em 6×, as seis
  ...
  **precisa estar escrito em negrito no contrato**, porque nada estoura: o número só fica errado.
  ...
  esse cálculo é do agregado do mês (leva 3), que é onde as somas do dashboard devem
  morar.
...
- **O filtro é a `CompetenceDate` da perna, se a etapa 10 já tiver subido** — ela grava a coluna
  ...
  revisitada quando o `CompetenceMode` chegar (leva 3). Se esta etapa vier primeiro,
...
> As etapas 10 e 11 e o **`CompetenceMode`, da leva 3**, saíram do mesmo debate, sobre como o cartão de
...
período, e a rotina mensal do orçamento, na leva 3 — passa a servir aos dois sem uma linha nova. Uma tabela
...
- **Pessoa arquivada** mantém o histórico de períodos e para de ser materializada, pela mesma
  regra que o `Budgets.Active` vai ganhar com a rotina mensal, na leva 3.
...
- **o "posso gastar"** (leva 3) não lê nenhum dos dois: gasto comprometido conta desde
...
repetir o `coalesce`, e o `CompetenceMode` (leva 3) vira só uma mudança **no que se
...
conciliação — que saiu da leva 3 em 2026-09-07 e continua fora de escopo.
...
- **`process.env.NODE_ENV` é lido direto em `AcessControl.section.ts`**, nos dois métodos de
  ...
  `enviromentManager.getEnv`. A etapa 1 mexe exatamente nesses métodos (e a duração de sessão
  configurável, na leva 3, mexe de novo); é a hora barata de alinhar.
