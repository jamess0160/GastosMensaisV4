...
posso gastar = OpeningBalance
             ...
             − PastCommitments                   (pesou antes, o dinheiro ainda está na conta)
...
### As duas bases de data, e o mês de estreia — correções de 2026-09-08

Três defeitos, um só de fundo: **a fórmula perdia ou dobrava a perna cuja data de peso e data de
caixa moram em meses diferentes**. Nenhum deles é de escala — todos aparecem com dois lançamentos
no banco, e o primeiro estava errado em produção.
...
**DECIDIDO: `PastCommitments` — a ponte entre a abertura, que é caixa, e o fluxo, que é
competência.** O `OpeningBalance` sai do `AccountBalance`, que corta pela `CashDate`; o resto da
fórmula corta pela `CompetenceDate`. Enquanto as duas coincidiam a soma fechava **por acidente**,
e foi assim que a etapa subiu. O `CompetenceMode` fez elas divergirem de propósito — e `purchase`
é o default de todo cartão, então a divergência virou a regra, não a exceção.

A perna que pesou em agosto e vence em setembro escapava por três buracos ao mesmo tempo, e o
terceiro é o pior: **quitar a fatura aumentava o "posso gastar"**, porque a perna paga com
competência anterior não estava na abertura (a `CashDate` é depois do corte), nem no fluxo do mês
(a competência é de antes), nem no vencido (estava paga). Ela sumia. Por isso o termo **não filtra
`Paid`**: o que decide é a data, nunca o estado.

**DECIDIDO: vencido exige as duas datas no passado.** Sem o recorte pela `CashDate` o
`OverduePayable` se sobrepõe ao `PastCommitments` e a mesma perna desce o indicador duas vezes —
e o rótulo mentia desde que o `CompetenceMode` existe: **toda** compra do mês passado no cartão
caía ali, e a tela anunciava como "R$ X vencidos" uma fatura que vence semana que vem. Mandar o
usuário atrás de uma pendência que não existe é pior do que não mostrar o número.

Com os três termos, cada perna é contada **exatamente uma vez** — e é essa partição, não a boa
vontade, que faz a soma fechar:

| A perna | Onde ela é contada |
| --- | --- |
| competência no mês | `Expenses` |
| competência anterior, caixa anterior, paga | já está no `OpeningBalance` |
| competência anterior, caixa anterior, não paga | `OverduePayable` |
| competência anterior, caixa daqui para a frente | `PastCommitments` |
| competência à frente | em lugar nenhum, que é o certo |

**DECIDIDO: a sobra do mês atravessa sozinha, e lançar a sobra à mão fica fora.** Terminar o mês 1
com 500 no indicador e ver o mês 2 abrir com 500 + o salário não é feature: é consequência da
partição acima. Vale a identidade

```
posso gastar(mês 2) = posso gastar(mês 1) + entradas do mês 2 − pernas do mês 2
```

O que sobrou e virou dinheiro está no `OpeningBalance` do mês seguinte; o que ficou pendente volta
pelo `OverdueReceivable`/`OverduePayable`; o que pesou antes e ainda não saiu volta pelo
`PastCommitments`. Não há o que transportar porque nada se perde no caminho.

E é por isso que **a opção manual não pode existir ao lado da automática**: cadastrar a sobra de
500 como entrada do mês 2 conta o mesmo dinheiro duas vezes — o indicador vai a 4000 com 500 na
conta. São os três argumentos do *Descartado: lançar o saldo como uma entrada* valendo igual para
a sobra: ela não tem data, não tem conta e não tem a quem atribuir. Oferecer as duas é garantir
que metade da base terá o número errado, sem nenhuma tela saber qual metade.
