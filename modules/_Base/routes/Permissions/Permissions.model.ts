...
export class class_Base_Permissions_model extends BaseModel {
    ...
    private readonly baseQuery = this.KnexConnection.select("*").from<BaseDatabase.Permissions>("Permissions").where("Active", 1).orderBy("IdPermission")

    getByGroups(IdGroupNames: number[]) {
        return this.baseQuery.clone().whereIn("IdUserGroupName", IdGroupNames)
    }

    create(records: MaybeArray<Partial<BaseDatabase.Permissions>>) {
    ...
    deletaAllFromGroup(IdUserGroupName: number) {
        return this.KnexConnection.update({ Active: 0 }).from<BaseDatabase.Permissions>("Permissions").where("IdUserGroupName", IdUserGroupName)
    }
}
