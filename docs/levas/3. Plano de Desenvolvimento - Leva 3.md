...
- **9 depois de 7**, por menos independente do que parece. A 9 é a abertura linha a linha do
  `AccountBalance`, e a etapa 7 tem em aberto exatamente **por qual data o saldo corta** — hoje a
  ...
  muda o extrato. Construir a 9 antes é construí-la duas vezes. Ela também divide pasta com a 8, e
...
- **O padrão do cartão novo é `'purchase'`**, porque o padrão tem que servir a quem não vai
  ...
  **Confirmar no momento da migration se existe base real:** pela leva 2, o cadastro
...
- **Ela herda a decisão de data da etapa 7, e por isso vem depois dela.** O corte das pernas pagas
  é hoje a `ExpensePayments.CompetenceDate`, e a etapa 7 tem em aberto se o saldo volta a cortar
...
**Ela não tem critério de fim, e não deveria fingir que tem.** Uma das dez etapas — a 10 — ainda
depende de decisão que ninguém tomou: qual biblioteca gera a planilha; e um critério escrito antes dessas respostas seria chute com cara de compromisso.
...
- **o que decidir antes** na única que falta, a 10.
...
**O critério de fim desta leva se escreve quando a primeira etapa dela começar** — que agora é
literalmente a 1, o motor de rotinas, que não espera nada além de si mesma.
