...
motor de rotinas é a etapa 9 e a infra de e-mail é a etapa 10. O que continua sendo dependência
...
notificações, saiu do MVP. Somam-se a elas **três demandas de produto** (as etapas 5, 8 e 11) e **a
infra que outras precisam** (as etapas 9 e 10), o que dá as dez etapas abaixo.

| Etapa | O que é | Pendência | O que a trava |
| --- | --- | --- | --- |
| **1** | [A rotina mensal do orçamento](#etapa-1--a-rotina-mensal-do-orçamento) | [4](../Pendencias%20Backend.md#4-rotina-mensal-de-orçamento) | a [etapa 9](#etapa-9--o-motor-de-rotinas) — é dentro do motor dela que a rotina roda |
...
| **3** | [Recuperação de senha](#etapa-3--recuperação-de-senha) | [6](../Pendencias%20Backend.md#6-recuperação-de-senha) | a [etapa 10](#etapa-10--a-infra-de-e-mail), que sobe junto |
...
| **8** | [Extrato de conta e de cartão](#etapa-8--extrato-de-conta-e-de-cartão) | — *demanda de produto* | a [etapa 5](#etapa-5--competencemode-o-cartão-que-conta-como-débito), de quem é a decisão de por qual data o saldo corta |
| **9** | [O motor de rotinas](#etapa-9--o-motor-de-rotinas) | — *infra* | nada; é a primeira da fila, porque a etapa 1 roda dentro dela |
| **10** | [A infra de e-mail](#etapa-10--a-infra-de-e-mail) | — *infra* | nada no código; o **provedor** com SPF/DKIM é externo |
| **11** | [Confirmação de e-mail](#etapa-11--confirmação-de-e-mail) | — *demanda de produto* | a etapa 10 |

**A numeração tem buraco, e é para ter.** O número 4 era das notificações, que saíram do MVP, e
**não será reaproveitado**: número aqui é identidade, e reusar o 4 faria "etapa 4 da leva 3"
querer dizer duas coisas conforme a data de quem lê. Pela mesma razão as etapas 9, 10 e 11 têm os
números mais altos sem serem as últimas da fila: a **9 abre a leva** e a 10 roda **antes da 3**.
A ordem está na seção seguinte, e é ela que vale — não a numeração.

Três das dez (2, 9 e 10) poderiam começar hoje. As outras sete esperam uma etapa anterior — a 1
espera a 9, a 3 e a 11 esperam a 10, a 5 e a 6 esperam a leva 2, a 7 espera a 6 e a 8 espera a 5 —,
e o valor deste documento está justamente em dizer **qual**, para ninguém abrir a etapa errada por
parecer a mais fácil.
...
graph TD
    ...
    E5 --> E6
    E9["9. Motor de rotinas"] --> E1["1. Rotina do orçamento<br/>(materializar + fechar o mês)"]
    E9 --> FIXO["extensão da janela<br/>do gasto fixo"]
    E6 --> E7["7. Exportar Excel"]
    E5 --> E8["8. Extrato"]
    E2["2. Sessão de 30 dias"]
    E10["10. Infra de e-mail"] --> E3["3. Recuperação de senha"]
    E10 --> E11["11. Confirmação de e-mail"]
    E10 -.->|"precisa de"| MAIL{{"provedor com<br/>SPF e DKIM"}}
```

- **9 antes de 1** — invertido em 2026-09-07. A etapa 1 vivia sem o motor por materialização
  preguiçosa no `GET`, e a razão disso era o projeto não ter agendador; como é esta leva que
  constrói o agendador, a premissa não sobrevive ao próprio documento. A 1 volta a ser o que o
  nome dela sempre disse — uma rotina — e vira o primeiro consumidor da 9, imediatamente depois
  dela: infra sem consumidor continua sem ter como ser validada, e agora a fila é de um.
...
- **10 antes de 3 e de 11.** A infra de e-mail nasce uma vez só e sobe **junto com a etapa 3**,
  que é o primeiro consumidor: a etapa 3 já tem desenho fechado e nenhuma decisão pendente, então
  é ela que valida a infra sem arrastar mais nada. A 11 vem depois, com a infra já provada.
- **2 é independente** de todo o resto; entra pela prioridade de produto.
- **8 depois de 5**, por menos independente do que parece. A 8 é a abertura linha a linha do
  `AccountBalance`, e a etapa 5 tem em aberto exatamente **por qual data o saldo corta** — hoje a
  `CompetenceDate` da perna, com duas saídas possíveis (voltar ao vencimento, ou a perna ganhar
  duas datas, competência e caixa). Qualquer uma das duas muda em que mês cada perna cai, ou seja,
  muda o extrato. Construir a 8 antes é construí-la duas vezes. Ela também divide pasta com a 6, e
  quem chegar primeiro cria `routes/Reports/`.
...
**Depende da [etapa 9](#etapa-9--o-motor-de-rotinas)**
...
agendador"*. Ele passa a ter, na **etapa 9 desta mesma leva** — a premissa da alternativa barata
...
Duas rotinas mensais, no dia 1º, registradas no motor da etapa 9 — e elas andam juntas porque são
...
O fechamento **estava adiado para a etapa 9** justamente por ser evento de tempo e não de leitura.
...
resolve é o catch-up da etapa 9, que roda a ocorrência vencida com o `ScheduledFor` do dia 1º.
...
- **Uma transaction por workspace**, como a etapa 9 exige de toda rotina: um workspace com dado
...
**Pendência 6** · **Pasta:** `routes/Users/` · **Sobe junto com a [etapa 10](#etapa-10--a-infra-de-e-mail)**, como primeiro consumidor dela
...
## Etapa 8 — Extrato de conta e de cartão

**Demanda de produto de 2026-09-07** · **Pasta:** `routes/Reports/` · **Substitui a conciliação**,
que saiu desta leva · **Depois da [etapa 5](#etapa-5--competencemode-o-cartão-que-conta-como-débito)**
...
Mora em `routes/Reports/`, junto com a etapa 6 — é a mesma natureza (leitura que atravessa várias
...
etapa 6. O que garante que as duas concordam não é a boa vontade — é o teste da etapa, abaixo.

O `OpeningBalance` é o mesmo termo da etapa 6, e sai do mesmo lugar: `AccountBalance` chamado com
...
| Corte | `CompetenceDate` no mês | `CompetenceDate` no mês, que no cartão é o vencimento |
...
dos dois lados — e continua valendo o que a etapa 6 já registrou: pagar a fatura não cria
lançamento nenhum.
...
- **Ela herda a decisão de data da etapa 5, e por isso vem depois dela.** O corte das pernas pagas
  é hoje a `ExpensePayments.CompetenceDate`, e a etapa 5 tem em aberto se o saldo volta a cortar
  pelo vencimento ou se a perna passa a ter duas datas. Como o extrato **é** a abertura do saldo,
  ele muda com o saldo: fechar isso antes é o que impede a etapa de ser escrita duas vezes. É a
  contrapartida da regra abaixo — se o extrato não pode inventar filtro próprio, ele também não
  pode ser desenhado antes de o filtro estar decidido.
...
| --- | --- | --- |
| Fechar o mês do orçamento (`Status='closed'`, `ClosedAt`) | todo dia 1º | etapa 1 |
| Materializar `BudgetPeriods` do mês | todo dia 1º | etapa 1 |
| Estender a janela do gasto fixo | mensal | leva 1, etapa 7: `Occurrences` gera 12 meses à frente e a corrente **acaba** |

As duas primeiras **são a etapa 1 inteira**, e é aqui que ela roda. Até 2026-09-07 a
...
sorteio de qual teto virou história. A etapa 1 registra o desfecho; aqui basta saber que a rotina
...
Essa inserção é o mesmo truque que a **etapa 1 já decidiu** usar para o `BudgetPeriods`, e aqui
...
- **A rotina delega, não reimplementa** — a mesma regra da etapa 6. Ela é um terceiro chamador ao
...
## Etapa 10 — A infra de e-mail

**Infra** — como a etapa 9, não sai de pendência: sai de três coisas que precisam mandar e-mail ·
**Arquivo novo:** `Utils/Connections/Mailer.ts` · **Pasta nova:** `Utils/Mail/templates/` ·
**Sobe junto com a [etapa 3](#etapa-3--recuperação-de-senha)**, que é o primeiro consumidor
...
rotina da etapa 9 lê o que venceu e não foi enviado, envia e carimba. Falta só `Attempts`/
`LastError`, se for retentar. Notificações saíram do MVP e esse dia não é agora.

Fila de verdade (`pg-boss`) continua descartada pelo mesmo motivo da etapa 9, agora com o gatilho
...
tela é que chama `POST /Users/resetPassword`. É o que a etapa 3 já desenhou ao receber o token no
...
- **Duas rotas públicas que disparam e-mail, e o projeto não tem rate limiting.** Já estava
  registrado na etapa 3; com o reenvio da etapa 11 viram duas. O remendo barato e nativo daqui é
...
- **Entregabilidade não é código.** Mandar como `@gastosmensais.com.br` por um SMTP não autorizado
  ...
  e-mail" que a etapa 3 declarava.
...
**Contrato:** **nenhuma entrada por si.** A infra não tem rota; quem traz rota é a etapa 3, que
sobe junto.

---

## Etapa 11 — Confirmação de e-mail

**Demanda de produto de 2026-09-07** · **Pasta:** `routes/Users/` · **Depois da etapa 10**
...
**DECIDIDO: o token é o mesmo padrão da etapa 3** — JWT curto, sem tabela, com `type: 'confirm'`
...
- **`resendConfirmation` responde `200` sempre**, inclusive para e-mail inexistente, pela mesma
  razão do `forgotPassword` da etapa 3: senão a rota vira um verificador de quais e-mails têm
  conta. E leva o mesmo cooldown do `cacheEngine`.
...
| Faturas de cartão como entidade | As datas de fatura já vivem na perna (`ClosingDate`/`DueDate`), e a etapa 8 mostra a fatura sem precisar de tabela para isso. Uma entidade própria só se pagaria com conciliação, que também saiu |
...
| **Conciliação de extrato** (pendência [7](../Pendencias%20Backend.md#7-conciliação-de-extrato)) | **Fora do MVP, decidido em 2026-09-07** — a etapa 8 passou a ser o extrato manual, que é o que a tela precisa agora. As três perguntas que travavam a conciliação continuam sem resposta e ficam registradas aqui: **de onde vem o extrato** (upload de OFX/CSV, ou Open Finance — a diferença é entre um parser e uma integração com credencial e homologação); **o que é um item "a resolver"** (lançamento no extrato sem par no app, par no app sem extrato, ou os dois com valores diferentes); e **se conciliar cria lançamento ou só marca os existentes** — se cria, é migration em `Inflows` e `Expenses` para dizer que aquele lançamento nasceu do extrato |

---

## Como saber que a leva acabou

**Ela não tem critério de fim, e não deveria fingir que tem.** Uma das dez etapas ainda depende de
decisão que ninguém tomou — qual biblioteca gera a planilha —, e um critério escrito antes dessas respostas seria chute com cara de compromisso.

O que esta leva tem, e é o que ela precisa ter agora:

- **ordem** — 1 antes de 9, 10 antes de 3 e de 11, as etapas 7 e 10 da leva 2 antes de 5 e 6, 6
  antes de 7;
- **desenho fechado** em nove das dez, com o porquê registrado;
- **o que decidir antes** na única que falta, a 7.
